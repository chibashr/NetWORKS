...
from plugins.template_manager.core.template_engine import (
    STANDARD_DEVICE_PROPERTIES,
    render_template_text,
    render_template_text_to_html,
)
...
class TemplateManagerPanel(QWidget):
    ...
    def __init__(self, plugin, parent=None):
        ...
        self.body_edit.textChanged.connect(self._refresh_preview_text)
        self.preview_device_combo.currentIndexChanged.connect(self._refresh_preview_text)
    ...
    def _build_ui(self):
        ...
        # Row 1: Templates | Template Details (same line)
        row1 = QHBoxLayout()
        ...
        row1.addWidget(templates_group)
        ...
        form_layout.addWidget(vars_group)
        ...
        self.send_cm_btn.clicked.connect(self._open_export_dialog)
        self.batch_export_btn = QPushButton("Batch export…")
        self.batch_export_btn.clicked.connect(self._open_batch_export_dialog)
        export_btn_row.addWidget(self.export_file_btn)
        export_btn_row.addWidget(self.send_cm_btn)
        export_btn_row.addWidget(self.batch_export_btn)
        ...
        preview_inner.addWidget(QLabel("Device:"))
        self.preview_device_combo = QComboBox()
        self.preview_device_combo.setMinimumWidth(200)
        preview_inner.addWidget(self.preview_device_combo)
        preview_inner.addWidget(QLabel("Rendered output:"))
        self.preview_text = QTextEdit()
        self.preview_text.setReadOnly(True)
        self.preview_text.setPlaceholderText("Select a device to see template rendered with its data. Select text here to locate it in the editor.")
        self.preview_text.selectionChanged.connect(self._on_preview_selection_changed)
    ...
    def _refresh_preview_devices(self):
        ...
        self.preview_device_combo.addItem("(No device)", None)
        if dm:
            for d in (dm.get_devices() or []):
                name = _device_display_name(d)
                self.preview_device_combo.addItem(name, d)
        self._refresh_preview_text()

    def _refresh_preview_text(self):
        body = self.body_edit.toPlainText().strip()
        device = self.preview_device_combo.currentData()
        if not body or not device:
            self.preview_text.setPlainText(body or "(Enter template body and select a device.)")
            return
        ctx = dict(device.get_properties() or {})
        try:
            html_out = render_template_text_to_html(body, ctx)
            self.preview_text.setHtml(html_out)
        except Exception:
            self.preview_text.setPlainText("(Preview error)")
    ...
    def _on_preview_selection_changed(self):
        ...
        if not sel or sel.startswith("\u2029"):  # Qt uses U+2029 for newline in selectedText()
            return
        plain = sel.replace("\u2029", "\n")
    ...
    def _open_export_dialog(self):
        ...
        if not to_export:
            QMessageBox.information(self, "Load into Command Manager", "No templates to export.")
            return
